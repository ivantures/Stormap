...
```{r map, echo = FALSE, output = 'asis', fig.width = 12, fig.height = 6}
source('../stormap-shiny/stormap-core.R', chdir = TRUE)

period <- select.period(ALL)
event.types <- select.event.types(ALL)
impact.types <- impact.types
events <- events.subset(period, event.types, impact.types)

events.plot(list(period = period,
